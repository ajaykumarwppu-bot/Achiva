...
class MainActivity : Activity() {
    ...
    /* THOUGHT-FIX : <input type="file"> (audio upload) ka result callback */
    private var fileCallback: ValueCallback<Array<android.net.Uri>>? = null
    private val FILE_REQ = 2002
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            /* THOUGHT-FIX : WebView mein recording + upload ke liye
               WebChromeClient zaroori hai —
               • onPermissionRequest : getUserMedia mic permission grant
               • onShowFileChooser  : <input type=file> ka audio picker */
    ...
    /* THOUGHT-FIX : file picker ka result web ke callback tak pahunchao */
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: android.content.Intent?) {
        if (requestCode == FILE_REQ) {
            val cb = fileCallback
            fileCallback = null
            val res = if (resultCode == Activity.RESULT_OK && data != null && data.data != null)
                arrayOf(data.data!!) else null
            try { cb?.onReceiveValue(res) } catch (_: Throwable) { }
        } else {
