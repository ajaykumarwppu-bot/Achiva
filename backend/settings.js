/* ================================================================
   BACKEND / SETTINGS.JS — header ka gear button + Settings sheet
   ----------------------------------------------------------------
   Responsibility:
     • Header ka purana theme-toggle button ab SETTINGS (gear) button
       hai. Theme toggle andar Settings sheet mein chala gaya hai.
     • Settings sheet mein:
...
(function () {
  ...
  function build() {
    ...
    scrim.style.cssText = 'position:absolute;inset:0;z-index:399;background:rgba(20,22,26,.45);' +
    ...
    panel.style.cssText = 'position:absolute;left:10px;right:10px;bottom:10px;z-index:400;' +
      'max-height:82%;overflow:auto;padding:14px 18px 20px;border-radius:24px;' +
      'background:var(--sheet-bg);border:1px solid var(--line);' +
      'box-shadow:0 30px 60px -24px rgba(10,12,16,.6),inset 0 1px 0 var(--hl-soft);' +
      'transform:translateY(115%);transition:transform .3s cubic-bezier(.32,.72,.24,1)';
    ...
    h3.style.cssText = 'font-family:var(--f-disp);font-size:18px;font-weight:700;color:var(--ink);margin:0';
    h3.textContent = 'Settings';
    var closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.setAttribute('aria-label', 'Band karein');
    closeBtn.style.cssText = 'width:26px;height:26px;border-radius:8px;border:0;background:transparent;' +
      'cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--slate)';
    closeBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" ' +
    ...
    panel.appendChild(buildTheme());
    panel.appendChild(buildBackup());
    panel.appendChild(buildAccount());
  ...
  function open() {
    ...
    window.requestAnimationFrame(function () { panel.style.transform = 'translateY(0)'; });
  }
